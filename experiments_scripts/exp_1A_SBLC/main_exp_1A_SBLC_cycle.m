...
signatures_store = cell(numRuns,1);
...
T = load(['pascal_filters_'  sprintf('%i', int32(iii)) '.mat']);
...
mDist1M_store = [mDist1M_store mean(mDist1)];
mDist1V_store = [mDist1V_store var(mDist1,1)];
...
mDist2M_store = [mDist2M_store mean(mDist2)];
mDist2V_store = [mDist2V_store var(mDist2,1)];
...
mDistCrossM_store = [mDistCrossM_store mean(mDistCross)];
mDistCrossV_store = [mDistCrossV_store var(mDistCross,1)];
...
empErrM_store = [ empErrM_store mean(missErr) ];
empErrV_store = [ empErrV_store var(missErr,1) ];
...
% Classification
figure
bar(empErrM_store); hold on;
errorbar(empErrM_store , sqrt(empErrV_store),'rx');
ylim([0,1]);
hold off;
...
figure
hold on;
plot(mDist1M_store, 'r');
plot(mDist2M_store), 'b';
plot(mDistCrossM_store, 'g');
% errorbar(mDist1M_store , sqrt(mDist1V_store),'rx');
% errorbar(mDist2M_store , sqrt(mDist2V_store),'rx');
% errorbar(mDistCrossM_store , sqrt(mDistCrossV_store),'rx');
ylim([0,1]);
hold off;